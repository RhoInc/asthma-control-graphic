var interactiveSankey = function (webcharts) {
    ...
    function syncSettings(settings) {
        ...
        settings.y.label = '# of Participants';
    ...
    function onResize() {
        ...
        barGroups.each(function (barGroup, i) {
            ...
            d3.select(this).selectAll('rect.wc-data-mark').each(function (d) {
                ...
                .append('text').datum([{ node: d.values.x, link: d.key, text: n + ' (' + d3.format('%')(pct) + ')' }])
                ...
                bar.select('title').text(n + ' participants at ' + d.key + ' (' + d3.format('%')(pct) + ")")
                //'):' + '\n - ' + IDs.slice(0, 3).join('\n - ') + (n > 3 ? '\n - and ' + (n - 3) + ' more' : ''));
                yPosition -= n;
        ...
        bars.style('cursor', 'pointer').on('click', function (d) {
            ...
            selectedIDbars.each(function () {
                d3.select(this).append('title').text(di => di.values.y + ' participants at ' + di.values.x + ' (' + d3.format('%')(di.values.y / selectedIDs.length) + ')')
            ...
            context.svg.selectAll('text.selectedIDs').data(selectedBarData).enter().append('text').attr({ class: 'selectedIDs',
                ...
                dx: '.25em',
                dy: '.9em' })
            .text(d => d.values.y / selectedIDs.length == 1 ? d.values.y : d.values.y + ' (' + d3.format('%')(d.values.y / selectedIDs.length) + ')');
            //Draw links.
